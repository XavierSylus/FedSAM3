...
class FederatedConfig:
    ...
    persist_client_optimizer: bool = False
    """是否跨轮保留客户端优化器状态；严格对照固定为 False"""
    ...
    def __post_init__(self):
        ...
        if self.seg_head_lr <= 0:
            raise ValueError(f"seg_head_lr 必须大于 0，当前值: {self.seg_head_lr}")
        if self.adapter_lr <= 0:
            raise ValueError(f"adapter_lr 必须大于 0，当前值: {self.adapter_lr}")
        if self.weight_decay < 0:
            raise ValueError(
                f"weight_decay 不能为负数，当前值: {self.weight_decay}"
            )
        ...
        if self.client_init_policy != "round_global":
            raise ValueError(
                "client_init_policy 必须为 round_global，以保证 2x2 对照只改变 routing 和 FedProx"
            )
        if self.persist_client_optimizer:
            raise ValueError(
                "persist_client_optimizer 必须为 False，以避免跨轮优化器状态混杂 FedProx 对照"
            )
    ...
    def from_yaml(cls, path: str) -> 'FederatedConfig':
        ...
        if 'training' in config_dict:
            ...
            flattened['seg_head_lr'] = training.get('seg_head_lr', 1e-3)
            flattened['adapter_lr'] = training.get('adapter_lr', 1e-4)
            flattened['weight_decay'] = training.get('weight_decay', 0.01)
